...
const float BALL_RESTITUTION = 0.05f;
const float BALL_ANGULAR_DAMPING = 0.80f; // 1/s (inverse seconds)
const float BALL_ISGROUNDED_NORMAL_THRESHOLD = 0.3f; // the y-coord of the normalized contact normal. 1.f = vertical wall, 0.f = flat ground
const float BALL_NONGRAPPLE_SOFT_SPEED_CAP = 300.f;
const float BALL_DRAG_COEFFICIENT = 40.0f;
const float RAMSTER_FLAME_THRESHOLD = 1000.f;

// grapple constants
const float GRAPPLE_STRAFE_SPEED_MULTIPLIER = 10.0f;
const float GRAPPLE_RANGE = 850.f;
...
//GRAPPLE PHYSICS
const float GRAPPLE_RETRACT_RATE =  20.0f;
